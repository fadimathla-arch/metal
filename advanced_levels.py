...
Expected input: list of candles where each candle is a dict with keys: 'time','open','high','low','close','volume'.
...
def detect_levels(candles: List[Dict[str, Any]], lookback: int = 20) -> Dict[str, float]:
    ...
    recent_low = extrem a = extrema['recent_low']
    recent_high = extrema['recent_high']
    major_low = extrema['major_low']
    major_high = extrema['major_high']

    # Order blocks: small zones around recent extremes
    ob_buy = round(recent_low + (major_low - recent_low) * 0.15, 2)
    ob_sell = round(recent_high + (major_high - recent_high) * 0.15, 2)
