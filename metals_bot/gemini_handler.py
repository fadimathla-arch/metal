...
def extract_and_save_signals(
    ...
    يُعيد (saved_count, gemini_report_text_or_None)
    """
    ...
    # أولاً: حاول استخراج JSON مهيكل داخل كودية ```json ... ``` أو مباشرة JSON يحتوي على مفتاح signals
    json_match = re.search(r"```json\s*(\{.*?\})\s*```", text, re.DOTALL)
    parsed_json = None
    if json_match:
        try:
            parsed_json = json.loads(json_match.group(1))
            gemini_report = parsed_json.get("report") or None
            ...
            log.warning(f"⚠️ فشل تحليل JSON من Gemini: {e}")

    # ثانياً: fallback إلى نمط SIGNAL النّصي القديم (regex) للحفاظ على التوافق
    ...
    for m in matches:
        ...
        if confidence["decision"] == "REJECT":
            log.warning(
                f"⚠️ إشارة مرفوضة ({symbol} {direction}): {confidence['reason']}"
            )
            continue
        ...
        if not valid:
            log.warning(
                f"⚠️ إشارة مرفوضة ({symbol} {direction}): {reason}"
            )
            continue
        ...
        log.info(
            f"📊 {quality['recommendation']} | {symbol} {direction}"
        )

        if quality["score"] < 55:
            log.info(
                f"🚫 إشارة منخفضة الجودة ({quality['score']}/100 — POOR) — {symbol} {direction} مرفوضة."
            )
            continue
    ...
    if found == 0 and "SIGNAL:" in text.upper():
        log.warning(
            "⚠️ وُجد SIGNAL: لكن لم يُستخرج — تحقق من تنسيق Gemini."
        )
    elif found > 0:
        log.info(
            f"📊 إشارات: وُجدت {found} | صالحة ومحفوظة: {saved}"
        )

    return saved, gemini_report
